...
let stats dim degs =
  ...
  let where =
    List.mapi (fun i deg ->
        sprintf
          "v.p%d = p%d.rowid AND p%d.dimension = %d AND p%d.degree = %d"
          i i i dim i deg) degs
  in
  ...
  Hashtbl.iter (fun row nb ->
      Printf.printf "%s, %s => %d %5.2f%%\n" row.(0) row.(1) nb (float nb /. float !total *. 100.0)) tbl
